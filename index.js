...
function diagnose(intent, session, callback){
   ...
   if (userAge == -1 && userGender == ""){
      ...
      api.getDiagnosis(symptomsIDs, userGender, year-userAge, function(data){
          ...
          speechOutput += " Thank you for using Diagnose Me. Have a nice day!!";
          callback({
            Symptoms: listOfSymptoms,
            Age: userAge,
            Gender: userGender
...
function handleInvalidInput(intent, session, callback){
    ...
    var listOfSymptoms = session.attributes ? session.attributes.Symptoms : [];
    var userAge = session.attributes ? session.attributes.Age : -1;
    var userGender = session.attributes ? session.attributes.Gender : "";
...
function prescribeMedication(illness, age, listOfSymptoms){//finish writing
    ...
    if(illness == "cold" && listOfSymptoms == ['cough']){
